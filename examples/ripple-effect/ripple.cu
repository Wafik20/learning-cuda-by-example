...
__global__ void drawRippleKernel(uint32_t *bitmap, int ticks)
{
    ...
    if (x < DIM_X && y < DIM_Y)
    {
        ...
        float ripple = cosf(d / 10.0f - ticks / 7.0f) + 0.5f * sinf(d / 5.0f - ticks / 3.5f) + 0.25f * cosf(d / 2.0f - ticks / 1.2f) + 0.15f * sinf(logf(d + 1.0f) + ticks / 10.0f);
...
int main(void)
{
    ...
    // Malloc the grid on the device
    uint32_t *d_bitmap;
    cudaMalloc(&d_bitmap, DIM_X * DIM_Y * sizeof(uint32_t));

    // zero the gpu bitmap
    cudaMemset(d_bitmap, 0, DIM_X * DIM_Y * sizeof(uint32_t));
    ...
    while (running)
    {
        ...
        drawRippleKernel<<<gridSize, blockSize>>>(d_bitmap, ticks++);
